...
(function ($) {
	...
	alan.isMobile = {
		...
		any: function () {
			return (alan.isMobile.Android() || alan.isMobile.BlackBerry() || alan.isMobile.iOS() || alan.isMobile.Opera() || alan.isMobile.Windows());
		}
	};

	alan.isHighDensity = function () {
	...
	alan.main = {
		...
		init: function () {
			alan.main.responsiveClasses();
			alan.main.pageTransition();
			alan.main.imageFade();
			alan.main.animations();
			alan.main.toggles();
			alan.main.accordions();
			alan.main.linkScroll();
			alan.main.lightBox();
			alan.main.resizeVideos();
			$body.on('lightBoxAjaxAdded', function () {
				alan.main.resizeVideos();
			});
			alan.main.loadFlexSlider();

			alan.main.applyRetina();
			alan.main.ajaxLogin();
			alan.main.ajaxLostPass();
			alan.main.menuWidget();

			/* Ajax search functionality */
			if ($('.alan_ajax_results').length) {
				alan.main.toggleAjaxSearch();
				alan.main.doAjaxSearch();
			}

			if (alan.isMobile.any()) {
		...
		linkScroll: function () {
			$("a[data-scrollto]").click(function () {
				...
				if (!divScrollOffset) {
					divScrollOffset = alan.main.topScrollOffset();
				}
				if (!divScrollEasing) {
		...
		doAjaxSearch: function (options) {
			...
			this.bind_ev = function () {
				...
				$(".ajax_s").on('click focus', function () {
					...
					var res = $(this).closest(".alan-search-form").find(".alan_ajax_results");
				...
				this.body.on('mousedown', function (e) {
					...
					if (!element.is('.alan_ajax_results, .ajax_s') && element.closest('.alan_ajax_results').length == 0) {
						$(".alan-search-form .alan_ajax_results").css("opacity", "0.5").slideUp('slow');
					}
				});
			...
			this.search = function (e) {
				...
				var $this = this,
					...
					results = form.find(".alan_ajax_results"),
					loading = form.children("button"),
					values = form.serialize();

				values += "&action=alan_ajax_search";
		...
		ajaxLogin: function () {
			...
			$("#alan-login-modal.mfp-hide .username").removeAttr("name");
			
			if ($("#alan-login-modal .alan-login-wrap").length) {

				$('.show-login, .alan-show-login, .bp-menu.bp-login-nav a, .must-log-in > a').magnificPopup({
					items: {
						src: '#alan-login-modal',
						type: 'inline',
						focus: '.username'
					},
					preloader: false,
					mainClass: 'alan-mfp-zoom',

					// When element is focused, some mobile browsers in some cases zoom in
					// It looks not nice, so we disable it:
					callbacks: {
						beforeOpen: function () {
							$("#alan-login-modal .username").attr("name", "log");
				...
				$('.show-login, .alan-show-login, .bp-menu.bp-login-nav a, .must-log-in > a').on('click', function () {
					$.magnificPopup.close();
				})
			...
			$('form.sq-login-form').on('submit', function (e) {
				...
				var theForm = $(this);
				var resultEl = theForm.find(".alan-login-result");

				var values = $(this).serialize();
				values += "&action=alanajaxlogin";


				resultEl.show().html(alan.loadingMessage);
				$.ajax({
					type: 'POST',
					dataType: 'json',
					url: alan.loginUrl,
		...
		ajaxLostPass: function () {
			...
			$(".sq-forgot-form").on("submit", function () {
				...
				var theForm = $(this);
				var resultEl = theForm.find(".alan-lost-result");
				...
				$.ajax({
					...
					error: function () {
						resultEl.html(alan.errorMessage).css('color', 'red');
					}
	...
	alan.header = {
		init: function () {
			alan.header.adminBarScrollClass();
			...
			$('.sidemenu-trigger').on("click touchstart",function () {
				...
				if ( $('body').hasClass("device-md") || $('body').hasClass("device-lg")) {
					if ($('body').hasClass('sidemenu-is-open')) {
						alan.main.setCookie('alan-side', 'open', '/', 30);
					} else {
						alan.main.setCookie('alan-side', '', '/', -1);
					}
				}
			...
			$("body").on('click', ".has-submenu .menu-arrow, .has-submenu > a[href='#']", function (e) {
				...
				if ( ! $(this).closest("li").parent(".basic-menu").length || $(this).closest('.click-menu').length || alan.isMobile.tabletWidth() || alan.isMobile.any() ) {
					alan.header.menuDropdown(this);
				}
				e.stopImmediatePropagation();
	...
	alan.bp = {
		init: function () {
			...
			if ($(".alan-notifications-nav").length) {
				$("#header").on("click", ".alan-notifications-nav a.mark-as-read", function (e) {
					alan.bp.notificationsRead($(this));
					e.preventDefault();
				});
			}
			if ($(".alan-notifications-nav").length || $(".alan-messages-nav").length) {
				if (alan.hasOwnProperty('bpAjaxRefresh') && alan.bpAjaxRefresh != '0') {
					alan.bp.ajaxCalls();
				}
			}
		},
		...
		ajaxCalls: function () {
			...
			alan.bp.rehreshID = setInterval(function () {

				var values = 'action=alan_bp_ajax_call';
				if ($(".alan-notifications-nav").length) {
					values += '&current_notifications=' + $(".alan-notifications b").first().text();
				}
				if ($(".alan-messages-nav").length) {
					values += '&current_messages=' + $(".alan-messages-nav b").first().text();
				}
					...
					success: function (response) {
						...
						if (response.statusNotif == 'success') {
							if (response.countNotif == '0') {
								$('.alan-notifications-nav .footer-item').hide();
								$(".alan-notifications-nav b").removeClass("new-alert").addClass("no-alert");
								$(".alan-notifications-nav .submenu").removeClass("has-notif");
							} else {
								$('.alan-notifications-nav').addClass("alan-loading");
								$(".alan-notifications-nav b").removeClass("no-alert").addClass("new-alert");
								$(".alan-notifications-nav .submenu").addClass("has-notif");
								$('.alan-notifications-nav .footer-item').show();
							}

							$(".alan-notifications-nav b").text(response.countNotif);
							$('.alan-notifications-nav .alan-submenu-item').remove();
							$('.alan-notifications-nav .submenu').prepend(response.dataNotif);
						} else {
							//
						}
						...
						if (response.statusMessages == 'success') {
							if (response.countMessagew == '0') {
								$('.alan-messages-nav .footer-item').hide();
								$(".alan-messages-nav b").removeClass("new-alert").addClass("no-alert");
								$(".alan-messages-nav .submenu").removeClass("has-notif");
							} else {
								$('.alan-messages-nav').addClass("alan-loading");
								$(".alan-messages-nav b").removeClass("no-alert").addClass("new-alert");
								$(".alan-messages-nav .submenu").addClass("has-notif");
								$('.alan-messages-nav .footer-item').show();
							}

							$(".alan-messages-nav b").text(response.countMessages);
							$('.alan-messages-nav .alan-submenu-item').remove();
							$('.alan-messages-nav .submenu').prepend(response.dataMessages);
						} else {
							//
						}
		...
		notificationsRead: function (e) {
			...
			var values = {action: "alan_bp_notification_mark_read"};
			...
			$.ajax({
				...
				success: function (response) {
					if (response.status == 'success') {
						if (response.count == '0') {
							$('.alan-notifications-nav .alan-submenu-item').remove();
							$('.alan-notifications-nav .submenu').prepend(response.empty);
							$('.alan-notifications-nav .footer-item').hide();
							$(".alan-notifications-nav b").removeClass("new-alert").addClass("no-alert");
							$(".alan-notifications-nav .submenu").removeClass("has-notif");
						} else {
						...
						$(".alan-notifications-nav b").text(response.count);
					} else {
						//
					}
				...
				complete: function () {
					$(".alan-notifications-nav").removeClass("alan-loading");
				}
			});
...
(function ($) {
	$.fn.flexMenu = function (options) {
		...
		return this.each(function () {
			...
			if (needsMenu($lastItem) && numItems > s.threshold && !s.undo && $this.is(':visible') && ! alan.isMobile.tabletWidth() ) {
		...
		if ( $('.header-icons > li').length > 1 ) {
			$('.header-icons').flexMenu({
				/*showOnHover: function () {
					return !alan.isMobile.tabletWidth();
				}*/
				showOnHover: false,
				popupAbsolute : false
			});
